import { useState } from 'react';
import { NavLink, Outlet, Navigate } from 'react-router-dom';
import {
  HomeIcon,
  BuildingLibraryIcon,
  ClipboardDocumentListIcon,
  DocumentCheckIcon,
  AcademicCapIcon,
  UserCircleIcon,
  Bars3Icon,
  XMarkIcon,
  SparklesIcon,
  ShieldCheckIcon,
} from '@heroicons/react/24/outline';
import Logo from '../components/Logo';
import { useAuth } from '../context/AuthContext';

const NAV_ITEMS = [
  { to: '/dashboard', label: 'Dashboard', Icon: HomeIcon },
  { to: '/dashboard/universities', label: 'Universities', Icon: BuildingLibraryIcon },
  { to: '/dashboard/tracker', label: 'Application Tracker', Icon: ClipboardDocumentListIcon },
  { to: '/dashboard/documents', label: 'Document Checklist', Icon: DocumentCheckIcon },
  { to: '/dashboard/scholarships', label: 'Scholarships', Icon: AcademicCapIcon },
  { to: '/dashboard/account', label: 'My Account', Icon: UserCircleIcon },
];

function SidebarContent({ onClose }: { onClose?: () => void }) {
  const { user, logout } = useAuth();

  const navClass = ({ isActive }: { isActive: boolean }) =>
    `flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all ${
      isActive
        ? 'bg-primary text-gray-900 shadow-sm shadow-amber-400/30'
        : 'text-slate-400 hover:bg-white/10 hover:text-white'
    }`;

  return (
    <div className="flex flex-col h-full">
      {/* Logo area */}
      <div className="p-5 border-b border-white/10 flex items-center justify-between">
        <Logo dark />
        {onClose && (
          <button onClick={onClose} className="p-1 rounded-lg hover:bg-white/10 lg:hidden">
            <XMarkIcon className="h-5 w-5 text-slate-400" />
          </button>
        )}
      </div>

      {/* Nav */}
      <nav className="flex-1 p-4 space-y-0.5 overflow-y-auto">
        {NAV_ITEMS.map(({ to, label, Icon }) => (
          <NavLink key={to} to={to} end={to === '/dashboard'} className={navClass} onClick={onClose}>
            <Icon className="h-5 w-5 flex-shrink-0" />
            {label}
          </NavLink>
        ))}
      </nav>

      {/* Bottom user area */}
      <div className="p-4 border-t border-white/10">
        {/* Upgrade CTA — non-premium only */}
        {!user?.is_premium && (
          <NavLink
            to="/dashboard/upgrade"
            className={({ isActive }) =>
              `flex items-center gap-2 mb-3 px-4 py-2.5 rounded-xl border text-sm font-semibold transition ${
                isActive
                  ? 'bg-amber-400/25 border-amber-400/40 text-amber-200'
                  : 'bg-gradient-to-r from-amber-400/15 to-yellow-300/10 border-amber-400/20 text-amber-300 hover:from-amber-400/25 hover:to-yellow-300/15'
              }`
            }
          >
            <SparklesIcon className="h-4 w-4 flex-shrink-0" />
            Upgrade to Premium
          </NavLink>
        )}

        {user?.is_premium ? (
          <div className="mb-3 px-3 py-2 bg-gradient-to-r from-amber-400/20 to-yellow-300/10 rounded-xl border border-amber-400/20">
            <div className="flex items-center gap-2">
              <SparklesIcon className="h-4 w-4 text-amber-400" />
              <span className="text-xs font-semibold text-amber-300">Premium</span>
            </div>
            <p className="text-xs text-slate-400 truncate mt-0.5">{user?.email}</p>
          </div>
        ) : (
          <div className="mb-3 px-3">
            <p className="text-xs text-slate-500 truncate">{user?.email}</p>
          </div>
        )}
        {/* Admin panel shortcut */}
        {user?.role === 'admin' && (
          <NavLink
            to="/admin"
            className={({ isActive }) =>
              `flex items-center gap-2 mb-2 px-4 py-2.5 rounded-xl text-sm font-semibold transition ${
                isActive
                  ? 'bg-violet-500/20 border border-violet-400/30 text-violet-300'
                  : 'text-slate-400 hover:bg-white/10 hover:text-white'
              }`
            }
          >
            <ShieldCheckIcon className="h-4 w-4 flex-shrink-0" />
            Admin Panel
          </NavLink>
        )}

        <button
          onClick={logout}
          className="w-full text-left px-4 py-2 text-sm text-slate-500 hover:text-red-400 rounded-xl hover:bg-red-500/10 transition-colors"
        >
          Sign out
        </button>
      </div>
    </div>
  );
}

export default function DashboardLayout() {
  const { user, isLoading } = useAuth();
  const [mobileOpen, setMobileOpen] = useState(false);

  if (isLoading) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-slate-900">
        <div className="animate-spin rounded-full h-10 w-10 border-b-2 border-primary" />
      </div>
    );
  }

  if (!user) return <Navigate to="/login" replace />;

  return (
    <div className="flex min-h-screen bg-slate-50">
      {/* Desktop sidebar */}
      <aside className="hidden lg:flex flex-col fixed inset-y-0 left-0 w-64 bg-slate-900 z-30">
        <SidebarContent />
      </aside>

      {/* Mobile sidebar overlay */}
      {mobileOpen && (
        <div className="fixed inset-0 z-40 lg:hidden">
          <div className="absolute inset-0 bg-black/50" onClick={() => setMobileOpen(false)} />
          <aside className="relative flex flex-col w-64 h-full bg-slate-900 shadow-xl">
            <SidebarContent onClose={() => setMobileOpen(false)} />
          </aside>
        </div>
      )}

      {/* Main content */}
      <main className="flex-1 lg:ml-64 flex flex-col min-h-screen">
        {/* Mobile top bar */}
        <div className="lg:hidden flex items-center gap-3 px-4 py-3 bg-slate-900 sticky top-0 z-20">
          <button onClick={() => setMobileOpen(true)} className="p-1 rounded-lg hover:bg-white/10">
            <Bars3Icon className="h-6 w-6 text-white" />
          </button>
          <Logo dark />
        </div>
        <div className="flex-1 p-6 lg:p-8">
          <Outlet />
        </div>
      </main>
    </div>
  );
}
